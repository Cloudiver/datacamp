# ------ 创建透视表------------------
# 获取某列的日期  dataframe["column"].dt.month  /year/day

# Add a year column to temperatures
temperatures['year'] = temperatures['date'].dt.year

# Pivot avg_temp_c by country and city vs year
# values 参数是要显示的值, index 参数是需要作为行索引的值 (传入元组)  columns 参数是作为 列索引
...
输出: temp_by_country_city_vs_year
year                              2000    2001    2002    2003    2004  ...    2009    2010    2011    2012    2013
country       city                                                      ...                                        
Afghanistan   Kabul             15.823  15.848  15.715  15.133  16.128  ...  15.093  15.676  15.812  14.510  16.206
Angola        Luanda            24.410  24.427  24.791  24.867  24.216  ...  24.325  24.440  24.151  24.240  24.554
...
print(temp_by_country_city_vs_year.loc[('Egypt', 'Cairo'):('India', 'Delhi')])  # 传入元组, 返回DataFrame对象

# Subset in both directions at once
print(temp_by_country_city_vs_year.loc[('Egypt', 'Cairo'):('India', 'Delhi'), '2005':'2010'])  # 终止列包含在内
...
mean_temp_by_year = temp_by_country_city_vs_year.mean()   # 计算每年的平均温度

# Filter for the year that had the highest mean temp
print(mean_temp_by_year[mean_temp_by_year == mean_temp_by_year.max()])  # 根据逻辑条件判断

# Get the mean temp by city
mean_temp_by_city = temp_by_country_city_vs_year.mean(axis='columns')   # 每个城市的多年平均温度

# Filter for the city that had the lowest mean temp
print(mean_temp_by_city[mean_temp_by_city == mean_temp_by_city.min()])  # 选出城市中最低多年平均温度
